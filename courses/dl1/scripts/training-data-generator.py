...
parser.add_argument('-d', action="store", dest="dest_dir", default="test-data",
                    help="destination directory path to store the files, inside of which you'll have a test and train directory, inside that will have one directory per search term")
parser.add_argument('-s', action="store", dest="search_terms",
                    default="cats,dogs", help="comma-separated list of terms to search for")
parser.add_argument("-q", action="store", dest="quantity_per_term", type=int,
                    default=100, help="how many images to get for each search term (max 100)")
parser.add_argument("-v", action="store", dest="valid_pct", type=int, default=20,
                    help="how much of the data to set aside for validation set (percentage, e.g. '20')")
        ...
        # we have enough validation files so we can choose some at random and put them in the right dir
            ...
            for validation_file in files[:validation_set_size]:
                src_path = os.path.join(
                    dest_dir, search_term_dir, validation_file)
                # print( f'source: "{src_path}"' );
                dest_path = os.path.join(
                    dest_dir, VALID_DIR_NAME, search_term_dir, validation_file)
